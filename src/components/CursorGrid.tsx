...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Pointer position is buffered and only applied once per rendered
      // frame — pointermove can fire far faster than 60fps (some
      // mice/trackpads report 120-240Hz), and energizing on every raw
      // event caused visible stutter instead of the intended smooth trail.
      if (pendingMove) {
        energize(pendingMove[0], pendingMove[1]);
        pendingMove = null;
      }
    ...
    // Cached to avoid a synchronous layout read on every pointermove —
    // refreshed on resize/scroll instead, which is far cheaper on a page
    // full of other scroll-driven animations.
